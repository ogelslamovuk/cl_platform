...
export type Channel = "OWN" | "ByCard" | "TicketPro" | "Bezkassira" | "SellerPOS";
...
export type FeeCalculationMode = "currentRules" | "expandedCalculation";
...
export interface ComplianceFeeLineItem {
  label: string;
  parameter: string;
  rate: string;
  quantity: string;
...
export interface ComplianceFeeBreakdown {
  ...
  lineItems: ComplianceFeeLineItem[];
}
...
function normalizeFeeCalculationMode(value: unknown): FeeCalculationMode {
  return value === "expandedCalculation" ? "expandedCalculation" : "currentRules";
}

function normalizeBaseUnitAmount(value: unknown): number {
  const parsed = Number(value);
  if (!Number.isFinite(parsed) || parsed <= 0) return DEMO_BASE_UNIT_AMOUNT;
  return Number(Math.max(1, Math.min(1000, parsed)).toFixed(2));
}
...
const DEFAULT_RESELLERS: Omit<Reseller, "updatedAt">[] = [
  ...
  {
    resellerId: "reseller_bezkassira",
    name: "Безкассира",
    code: "Bezkassira",
    status: "active",
    admissionStatus: "Авторизован",
    ...
    contractStatus: "Active",
    commissionPercent: 8,
    fullName: "ООО «Безкассира»",
    registrationNumber: "193000444",
    legalAddress: "220030, г. Минск, ул. Кирова, 8, офис 12",
    contactPerson: "Елена Соколова",
    email: "integration@bezkassira.example",
    phone: "+375 (29) 700-40-40",
    contractNumber: "TH-BK-2026-03",
    contractDate: "2026-03-01",
    apiEndpoint: "https://sandbox.api.bezkassira.by/v2/tickethub",
    webhookEndpoint: "https://sandbox.bezkassira.by/webhooks/tickethub",
    signatureValidation: true,
...
const DEFAULT_SEAT_MAP_LAYOUTS: SeatMapLayout[] = [
  ...
  defaultLayout("layout_bolshoi_demo", "venue_bolshoi_theatre", "hall_bolshoi_demo", "Камерная схема 4x6", 4, 6),
  defaultLayout("layout_grodno_culture", "venue_grodno_culture", "hall_grodno_main", "Зал 4x5", 4, 5),
  defaultLayout("layout_demo_philharmonic", "venue_demo_philharmonic", "hall_demo_philharmonic", "Филармония · зал 12x20", 12, 20),
  defaultLayout("layout_vitebsk_kolas", "venue_vitebsk_kolas", "hall_vitebsk_kolas", "Театр Коласа · зал 12x18", 12, 18),
  defaultLayout("layout_gomel_culture_demo", "venue_gomel_culture_demo", "hall_gomel_culture_demo", "Гомельский центр · зал 10x14", 10, 14),
  defaultLayout("layout_palace_demo_chamber", "venue_palace_demo_chamber", "hall_palace_demo_chamber", "Дворец Республики · зал 16x20", 16, 20),
...
const DEFAULT_VENUE_REGISTRY: VenueRegistryRecord[] = [
  {
    ...
    region: "Минск",
    type: "open-air",
    address: "пл. Свободы, Верхний город",
    description: "Городская открытая площадка без схемы мест; продажа ведётся по зонам входа.",
    capacity: 1200,
    status: "approved",
    halls: [{ hallId: "hall_upper_city_open", venueId: "venue_upper_city_open", name: "Фестивальная зона", capacity: 1200, hasSeatMap: false }],
  },
  {
    ...
    region: "Минск",
    type: "концертный зал",
    address: "пр-т Независимости, 50",
    description: "Зал филармонии со схемой мест до 500 зрителей.",
    capacity: 240,
    status: "approved",
    halls: [{ hallId: "hall_demo_philharmonic", venueId: "venue_demo_philharmonic", name: "Большой зал", capacity: 240, hasSeatMap: true, layoutId: "layout_demo_philharmonic" }],
  },
  {
    venueId: "venue_vitebsk_kolas",
    name: "Национальный академический драматический театр имени Якуба Коласа",
    city: "Витебск",
    region: "Витебская область",
    type: "театр",
    address: "ул. Замковая, 2",
    description: "Театральная площадка с компактной рассадкой.",
    capacity: 216,
    status: "approved",
    halls: [{ hallId: "hall_vitebsk_kolas", venueId: "venue_vitebsk_kolas", name: "Основная сцена", capacity: 216, hasSeatMap: true, layoutId: "layout_vitebsk_kolas" }],
  },
  {
    venueId: "venue_gomel_culture_demo",
    name: "Гомельский городской центр культуры",
    city: "Гомель",
    region: "Гомельская область",
    type: "центр культуры",
    address: "ул. Советская, 20",
    description: "Малый зал для детских и семейных программ.",
    capacity: 140,
    status: "approved",
    halls: [{ hallId: "hall_gomel_culture_demo", venueId: "venue_gomel_culture_demo", name: "Малый зал", capacity: 140, hasSeatMap: true, layoutId: "layout_gomel_culture_demo" }],
  },
  {
    ...
    description: "Большой концертный зал Дворца Республики со схемой мест до 500 зрителей.",
    capacity: 320,
    status: "approved",
    halls: [{ hallId: "hall_palace_demo_chamber", venueId: "venue_palace_demo_chamber", name: "Большой зал", capacity: 320, hasSeatMap: true, layoutId: "layout_palace_demo_chamber" }],
  },
  {
    ...
    description: "Зал для ремесленных фестивалей и лекционных программ.",
    capacity: 144,
    status: "approved",
    halls: [{ hallId: "hall_slutsk_crafts", venueId: "venue_slutsk_crafts", name: "Зал ремёсел", capacity: 144, hasSeatMap: true, layoutId: "layout_slutsk_crafts" }],
  },
  {
    venueId: "venue_nesvizh_castle",
    name: "Замковый комплекс «Несвиж»",
    city: "Несвиж",
    region: "Минская область",
    type: "центр культуры",
    address: "ул. Замковая, 2",
    description: "Камерный зал историко-культурной площадки.",
    capacity: 112,
    status: "approved",
    halls: [{ hallId: "hall_nesvizh_castle", venueId: "venue_nesvizh_castle", name: "Камерный зал", capacity: 112, hasSeatMap: true, layoutId: "layout_nesvizh_castle" }],
  },
  {
    ...
    description: "Гродненская театральная площадка со схемой мест.",
    capacity: 252,
    status: "approved",
    halls: [{ hallId: "hall_grodno_drama", venueId: "venue_grodno_drama", name: "Основная сцена", capacity: 252, hasSeatMap: true, layoutId: "layout_grodno_drama" }],
  },
  {
    ...
    type: "центр культуры",
    address: "ул. Карла Маркса, 60",
    description: "Музейный зал для выставочных программ.",
    capacity: 120,
    status: "approved",
    halls: [{ hallId: "hall_brest_museum", venueId: "venue_brest_museum", name: "Выставочный зал", capacity: 120, hasSeatMap: true, layoutId: "layout_brest_museum" }],
  },
  {
    ...
    region: "Витебская область",
    type: "концертный зал",
    address: "пр-т Фрунзе, 1",
    description: "Концертный зал для конкурсных программ молодых исполнителей.",
    capacity: 192,
    status: "approved",
    halls: [{ hallId: "hall_vitebsk_concert_hall", venueId: "venue_vitebsk_concert_hall", name: "Основной зал", capacity: 192, hasSeatMap: true, layoutId: "layout_vitebsk_concert_hall" }],
  },
  {
    ...
    description: "Зал НЦСИ для международных культурных программ.",
    capacity: 300,
    status: "approved",
    halls: [{ hallId: "hall_ncca_minsk", venueId: "venue_ncca_minsk", name: "Многофункциональный зал", capacity: 300, hasSeatMap: true, layoutId: "layout_ncca_minsk" }],
  },
  {
  ...
  },
  {
    ...
    capacity: 24,
    status: "approved",
    halls: [{ hallId: "hall_bolshoi_demo", venueId: "venue_bolshoi_theatre", name: "Камерный зал", capacity: 24, hasSeatMap: true, layoutId: "layout_bolshoi_demo" }],
  },
  {
...
export function defaultState(): AppState {
  ...
  const state: AppState = {
    ...
    finance: {
      ...
      baseUnitAmountBYN: DEMO_BASE_UNIT_AMOUNT,
...
export function ensureOrganizerFinanceState(state: AppState): void {
  ...
  state.finance.platformCommissionPercent = normalizePlatformCommissionPercent(state.finance.platformCommissionPercent);
  state.finance.baseUnitAmountBYN = normalizeBaseUnitAmount(state.finance.baseUnitAmountBYN);
  state.finance.feeMode = normalizeFeeCalculationMode(state.finance.feeMode);
  ...
  try {
    const state: AppState = {
      ...
      finance: {
        ...
        baseUnitAmountBYN: normalizeBaseUnitAmount(parsed.finance?.baseUnitAmountBYN),
        feeMode: normalizeFeeCalculationMode(parsed.finance?.feeMode),
...
export function ensureSeatMapState(state: AppState): void {
  ...
  for (const venue of DEFAULT_VENUE_REGISTRY) {
    const existing = state.venueRegistry.find((item) => item.venueId === venue.venueId);
    if (!existing) {
...
export function createVenueRegistryRecord(state: AppState, input: CreateVenueRegistryInput): VenueRegistryRecord | null {
  ...
  const venue: VenueRegistryRecord = {
    ...
    type: input.type,
    address,
    description: input.description.trim() || "Площадка, созданная в реестре Центра Управления.",
    capacity,
    status: "approved",
    halls: [{
      hallId,
...
export function ensureMockVenueRegistryRecord(state: AppState): VenueRegistryRecord {
  ...
  const venueName = "Площадка со схемой зала";
  const hallName = "Камерный зал";
  ...
  if (!venue) {
    venue = {
      ...
      region: "Минск",
      type: "концертный зал",
      address: "ул. Культурная, 1",
      description: "Площадка для проверки схемы зала в реестре.",
      capacity,
      status: "approved",
      halls: [{
        hallId,
    ...
    venue.address ||= "ул. Культурная, 1";
    venue.description ||= "Площадка для проверки схемы зала в реестре.";
...
export function getFeeCalculationMode(state?: Pick<AppState, "finance"> | null): FeeCalculationMode {
  return normalizeFeeCalculationMode(state?.finance?.feeMode);
}

export function getFeeBaseUnitAmount(state?: Pick<AppState, "finance"> | null): number {
  return normalizeBaseUnitAmount(state?.finance?.baseUnitAmountBYN);
}

function getComplianceFeeBasis(data: EventComplianceData): { value: number; label: string; rangeLabel: string } {
  const tierTotal = sumTierQuantity(data.ticketTiers || []);
  const planned = data.plannedTicketsForSale && data.plannedTicketsForSale > 0 ? data.plannedTicketsForSale : 0;
  const capacity = data.projectedCapacity && data.projectedCapacity > 0 ? data.projectedCapacity : 0;
  const value = capacity || tierTotal || planned || 0;
  const unit = capacity ? "мест" : "входных билетов";
  const label = value > 0
    ? `${capacity ? "проектная вместимость" : "планируемое количество"} ${value} ${unit}`
    : "нет проектной вместимости, реализация билетов заявлена";
  let rangeLabel = "нет проектной вместимости, но реализуются билеты";
  if (value > 0 && value <= 150) rangeLabel = "1-150 мест / входных билетов";
  else if (value > 0 && value <= 300) rangeLabel = "151-300 мест / входных билетов";
  else if (value > 0 && value <= 500) rangeLabel = "301-500 мест / входных билетов";
  else if (value > 0 && value <= 1000) rangeLabel = "501-1000 мест / входных билетов";
  else if (value > 0 && value <= 1500) rangeLabel = "1001-1500 мест / входных билетов";
  else if (value > 0 && value <= 2000) rangeLabel = "1501-2000 мест / входных билетов";
  else if (value > 0 && value <= 3000) rangeLabel = "2001-3000 мест / входных билетов";
  else if (value > 3000) rangeLabel = "свыше 3000 мест / входных билетов";
  return { value, label, rangeLabel };
}

function countPerformerPeople(data: EventComplianceData): number {
  return (data.performers || []).reduce((sum, performer) => sum + (performer.members?.length || 1), 0);
}

export function buildComplianceFeeBreakdown(state: Pick<AppState, "finance"> | null | undefined, data: EventComplianceData): ComplianceFeeBreakdown {
  const baseUnitAmountBYN = getFeeBaseUnitAmount(state);
  const mode = getFeeCalculationMode(state);
  const basis = getComplianceFeeBasis(data);
  const rateBaseUnits = calculateComplianceFee(data.projectedCapacity, data.plannedTicketsForSale, data.ticketTiers);
  const baseAmount = Number((rateBaseUnits * baseUnitAmountBYN).toFixed(2));
  const isExempt = Boolean(data.feeExempt || data.approvalMode !== "certificate_required");
  const exemptionReason = data.feeExemptReason || (data.approvalMode !== "certificate_required" ? "удостоверение на проведение мероприятия не требуется" : "");
  ...
  if (isExempt) {
    return {
      ...
      rangeLabel: basis.rangeLabel,
      rateBaseUnits: 0,
      baseUnitAmountBYN,
      formula: "Госпошлина не требуется",
      totalAmount: 0,
      baseAmount: 0,
      additionalAmount: 0,
      lineItems: [],
  ...
  const baseLine: ComplianceFeeLineItem = {
    label: "Базовое рассмотрение заявки",
    parameter: basis.rangeLabel,
    rate: `${rateBaseUnits} БВ`,
    quantity: "1",
    amount: baseAmount,
  };

  if (mode === "currentRules") {
    return {
      mode,
      isExempt: false,
      exemptionReason: "",
      ...
      rateBaseUnits,
      baseUnitAmountBYN,
      formula: `${rateBaseUnits} × ${baseUnitAmountBYN} BYN = ${baseAmount} BYN`,
  ...
  const hasComplexScheme = Boolean(data.layoutId || data.eventSeats?.length);
  const hasForeignPerformers = Boolean(data.hasForeignPerformers || (data.performers || []).some((performer) => performer.country !== "Беларусь"));
  const salesChannelCount = Math.max(1, (data.salesChannels || []).length);
  const documentCount = (data.eventDocuments || []).length + (data.venueContractStatus === "приложен" ? 1 : 0);
  const performerCount = countPerformerPeople(data);
  const lineItems: ComplianceFeeLineItem[] = [
    baseLine,
    {
      label: "Проверка площадки и вместимости",
      parameter: data.venueType || data.venueName || "площадка из реестра",
      rate: "+10% к базовому начислению",
      quantity: "1",
      amount: Number((baseAmount * 0.1).toFixed(2)),
    },
    {
      label: "Проверка схемы зала / билетной модели",
      parameter: hasComplexScheme ? "схема мест и тарифы" : "без закреплённых мест",
      rate: hasComplexScheme ? "1 БВ" : "0 БВ",
      quantity: hasComplexScheme ? "1" : "0",
      amount: hasComplexScheme ? baseUnitAmountBYN : 0,
    },
    {
      label: "Проверка состава исполнителей",
      parameter: hasForeignPerformers ? "есть иностранные участники" : `${performerCount || 1} участник(ов)`,
      rate: hasForeignPerformers ? "2 БВ" : "1 БВ",
      quantity: "1",
      amount: hasForeignPerformers ? baseUnitAmountBYN * 2 : baseUnitAmountBYN,
    },
    {
      label: "Проверка материалов и документов",
      parameter: `${documentCount || 1} документ(ов)`,
      rate: "1 БВ",
      quantity: "1",
      amount: baseUnitAmountBYN,
    },
    {
      label: "Контроль билетных каналов",
      parameter: `${salesChannelCount} канал(а) продаж`,
      rate: salesChannelCount > 1 ? "1 БВ" : "0 БВ",
      quantity: salesChannelCount > 1 ? "1" : "0",
      amount: salesChannelCount > 1 ? baseUnitAmountBYN : 0,
    },
    {
      label: "Постсобытийная сверка продаж",
      parameter: "включена в демо-сценарий",
      rate: "1 БВ",
      quantity: "1",
      amount: baseUnitAmountBYN,
    },
  ...
  const additionalAmount = Number(Math.max(0, totalAmount - baseAmount).toFixed(2));
  ...
  return {
    ...
    rateBaseUnits,
    baseUnitAmountBYN,
    formula: `${rateBaseUnits} × ${baseUnitAmountBYN} BYN + ${additionalAmount} BYN = ${totalAmount} BYN`,
    totalAmount,
    baseAmount,
    additionalAmount,
    lineItems,
...
export function calculateComplianceFeeAmount(data: EventComplianceData, state?: Pick<AppState, "finance"> | null): number {
  if (data.feeExempt || data.approvalMode !== "certificate_required") return 0;
  return buildComplianceFeeBreakdown(state, data).totalAmount;
}
...
export function generateComplianceFeeReceipt(
  ...
  if (!app) return null;
  const amount = calculateComplianceFeeAmount(app.data, state);
...
export function setPlatformCommissionPercent(state: AppState, commissionPercent: number): boolean {
  ensureOrganizerFinanceState(state);
  state.finance.platformCommissionPercent = normalizePlatformCommissionPercent(commissionPercent);
  saveState(state);
  return true;
}

export function setFeeCalculationMode(state: AppState, mode: FeeCalculationMode): boolean {
  ensureOrganizerFinanceState(state);
  state.finance.feeMode = normalizeFeeCalculationMode(mode);
  saveState(state);
  return true;
}
...
export function createReseller(state: AppState, input: CreateResellerInput): { ok: boolean; reseller?: Reseller; reason?: string } {
  ...
  if (!name) return { ok: false, reason: "Название оператора обязательно." };
...
export function sellTicketsByReseller(
  ...
  if (!reseller || reseller.status === "disabled") {
    return { ok: false, reason: "Оператор отключён в Центре Управления. Продажа недоступна.", ticketIds: [] };
  }
  ...
  if (!reseller.apiConnected || reseller.contractStatus !== "Active") {
    return { ok: false, reason: "API или договор оператора не активны.", ticketIds: [] };
  }
  ...
  if (!isSalesChannelAllowedForEvent(state, event, reseller.code)) {
    return { ok: false, reason: "Оператор не выбран в каналах продаж этого мероприятия.", ticketIds: [] };
  }
  ...
  const now = nowIso();
  const buyer = data.buyerName.trim() || "Покупатель оператора";
...
export function refundTicketByReseller(state: AppState, data: { resellerCode: string; ticketId: string }): ResellerRefundOutcome {
  const reseller = state.resellers.find((item) => item.code === data.resellerCode);
  if (!reseller || reseller.status === "disabled") {
    return { ok: false, reason: "Оператор отключён в Центре Управления. Возврат недоступен." };
  }
  if (!isResellerAuthorizedForSales(reseller)) {
    return { ok: false, reason: getResellerSalesBlockReason(reseller) };
  }
  if (!reseller.apiConnected || reseller.contractStatus !== "Active") {
    return { ok: false, reason: "API или договор оператора не активны." };
  }
